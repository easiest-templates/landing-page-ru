...
function findProjectRoot(startPath = process.cwd()) {
  try {
    ...
    let count = 0;
    while (
      currentPath !== import_path.default.parse(currentPath).root &&
      count < 20
    ) {
      if (
        (0, import_fs.existsSync)(
          import_path.default.join(currentPath, "package.json")
        )
      ) {
        return currentPath;
      }
...
var tailwindInputFile = import_path2.default.resolve(
  projectRoot,
  "./tailwind.config.ts"
);
var tailwindJsonOutfile = import_path2.default.resolve(
  projectRoot,
  "./src/tailwind.config.pp.json"
);
var tailwindIntermediateFile = import_path2.default.resolve(
  projectRoot,
  "./.pp.tailwind.config.js"
);
var isSandbox = process.env.POEHALI_DEV_SERVER === "true";
function componentTagger() {
  ...
  return {
    ...
    async transform(code, id) {
      if (
        !validExtensions.has(import_path2.default.extname(id)) ||
            ...
            if (node.type === "JSXOpeningElement") {
              ...
              if (shouldTag) {
                ...
                magicString.appendLeft(
                  jsxNode.name.end ?? 0,
                  ` data-pp-id="${dataComponentId}" data-pp-name="${elementName}" ${legacyIds}`
                );
    ...
    async buildStart() {
      ...
      try {
        ...
        console.error("Error generating tailwind.config.pp.json:", error);
      }
    },
    configureServer(server) {
      ...
      try {
        ...
        server.watcher.on("change", async (changedPath) => {
          if (
            import_path2.default.normalize(changedPath) ===
            import_path2.default.normalize(tailwindInputFile)
          ) {
            await generateConfig();
          }
        });
...
async function generateConfig() {
  try {
    ...
    try {
      ...
      const resolvedConfig = (0, import_resolveConfig.default)(
        userConfig.default
      );
      ...
      await import_promises.default
        .unlink(tailwindIntermediateFile)
        .catch(() => {});
    } catch (error) {
